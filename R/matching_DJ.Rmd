...
################################  !  !  !  !  !
knitr::opts_knit$set(root.dir = "~/GitHub/ecls") # change working directory to ecls folder
################################  !  !  !  !  !
```
...
> "The goal of matching is to produce covariate balance, that is, for the distributions of covariates in the two groups to be approximately equal to each other, as they would be in a successful randomized experiment. The importance of covariate balance is that it allows for increased robustness to the choice of model used to estimate the treatment effect; in perfectly balanced samples, a simple difference in means can be a valid treatment effect estimate." ([MatchIt Vignette](https://cran.r-project.org/web/packages/MatchIt/vignettes/MatchIt.html))

> "A matching analysis involves four primary steps: 1) planning, 2) matching, 3) assessing the quality of matches, and 4) estimating the treatment effect and its uncertainty." ([MatchIt Vignette](https://cran.r-project.org/web/packages/MatchIt/vignettes/MatchIt.html))

Loading required packages (`install.packages()` first if necessary):
```{r, message=FALSE}
...
```{r}
x_match <- matchit(formula, method = "exact", distance = "glm", data = ecls_nomiss)
...
```{r}
c_match <- matchit(formula, method = "cem", distance = "glm", data = ecls_nomiss)
